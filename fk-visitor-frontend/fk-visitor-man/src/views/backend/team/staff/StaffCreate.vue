...
export default {
  ...
  methods: {
    ...
    handleSubmit () {
      this.$refs['staffCreate'].validate(async valid => {
        if (valid) {
          this.confirmLoading = true

          try {
            await TeamService.staff(this.id, this.form)
            this.handleClose()
            this.$emit('ok')
          } catch (e) { }
    ...
    handleClose () {
      if (this.$refs['staffCreate'] !== undefined) {
        this.$refs['staffCreate'].resetFields()
      }
      this.visible = false
    }
  }
