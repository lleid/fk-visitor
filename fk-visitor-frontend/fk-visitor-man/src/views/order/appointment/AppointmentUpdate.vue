<template>
  <c-modal title="编辑预约" centered :destroyOnClose="true" :visible="visible" :loading="loading" :confirmLoading="confirmLoading" @ok="handleSubmit" @cancel="handleClose">
    <a-form-model ref="appointmentUpdate" :model="form" :rules="rules" :labelCol="labelCol" :wrapperCol="wrapperCol" :validate-messages="validateMessages">
...
export default {
  ...
  methods: {
    ...
    async edit (record) {
      ...
      this.loading = true
      try {
        const purposes = await PurposeService.query({ isDeleted: false }, { showLoading: false })
        this.purposes = purposes

        const visitAreas = await VisitAreaService.query({ isDeleted: false }, { showLoading: false })
        ...
        this.form = result
        if (result.orderAt) {
          this.form.orderAt = moment(result.orderAt, 'YYYY-MM-DD')
        }
      ...
      this.$refs['appointmentUpdate'].validate(async valid => {
        if (valid) {
          ...
          const requestModel = {
            ...
            purpose: { id: this.form.purposeId },
            visitArea: { id: this.form.visitAreaId },
            isMessage: this.form.isMessage
          }

          if (this.form.orderAt) {
            requestModel.orderAt = this.form.orderAt.format('YYYY-MM-DD')
          }
