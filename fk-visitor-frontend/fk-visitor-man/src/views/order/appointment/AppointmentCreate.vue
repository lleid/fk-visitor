<template>
  <c-modal title="添加预约" centered :destroyOnClose="true" :visible="visible" :loading="loading" :confirmLoading="confirmLoading" @ok="handleSubmit" @cancel="handleClose">
    <a-form-model :label-col="labelCol" :wrapper-col="wrapperCol" ref="appointmentCreate" :model="form" :rules="rules" :validate-messages="validateMessages">
    ...
    handleSubmit () {
      this.$refs['appointmentCreate'].validate(async valid => {
        if (valid) {
          ...
          const requestModel = Object.assign({}, this.form)
          if (this.form.orderAt) {
            requestModel.orderAt = requestModel.orderAt.format('YYYY-MM-DD')
          }

          try {
            await AppointmentService.create(requestModel)
            this.handleClose()
            this.$emit('ok')
          } catch (e) { }
